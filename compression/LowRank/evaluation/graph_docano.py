...
for resolution in test_loaders.keys():
    ...
    # Plot lines
    plt.plot(np.array(lowrank_ratios) * 100, l2_errors, 
             f'-{markers[0]}', label=f'L2 Loss ({resolution}x{resolution})', 
             color=colors[0], alpha=0.7)
    plt.plot(np.array(lowrank_ratios) * 100, h1_errors, 
             f'-{markers[1]}', label=f'H1 Loss ({resolution}x{resolution})', 
             color=colors[1], alpha=0.7)

plt.xlabel('LowRank Ratio (%)')
plt.ylabel('Relative Error Increase (%)')
plt.title('Model Performance vs SVD Low Rank Ratio for Darcy Equation on DOCANO')
