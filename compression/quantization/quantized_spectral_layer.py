...
class QuantizedSpectralConv(BaseSpectralConv):
    ...
    def __init__(
        ...
        if weight.is_complex():
            ...
            q_real, scale_real, min_real = self._quantize_tensor_int8(weight.real)
            q_imag, scale_imag, min_imag = self._quantize_tensor_int8(weight.imag)
            self.register_buffer("q_real", q_real)
            self.register_buffer("q_imag", q_imag)
            self.scale_real = scale_real
            self.scale_imag = scale_imag
            self.register_buffer("min_real", torch.tensor(min_real, dtype=torch.float32))
            self.register_buffer("min_imag", torch.tensor(min_imag, dtype=torch.float32))
            self.register_buffer("q_weight", None)
        else:
            q_weight, w_scale, w_min = self._quantize_tensor_int8(weight)
        ...
        if self.has_bias and spectral_layer.bias is not None:
            q_bias, bias_scale, b_min = self._quantize_tensor_int8(spectral_layer.bias)
        ...
        # Get the contraction function (note: using the original weight for selecting contract fun)
        self._contract = get_contract_fun(spectral_layer.weight, implementation=implementation, separable=separable)
        ...
        # Instead of creating new weight tensor based on shape,
    ...
    def _quantize_tensor_int8(self, tensor: torch.Tensor):
        """
        Quantize a tensor to int8 using min-max scaling.
        Scale is computed as: (max - min) / 256, and we map values by:
           q = round((val - min) / scale - 127), clamped to [-127, 127].
        """
